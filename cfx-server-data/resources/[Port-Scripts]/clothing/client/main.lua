-- Need to create a draw text and hide text UI element for when in a zone.
...
RegisterNetEvent('MP:Client:OnPlayerLoaded', function()
    TriggerServerEvent("qb-clothes:loadPlayerSkin")
    PlayerData = MP.Functions.GetPlayerData()
end)

-- RegisterNetEvent('MP:Client:OnJobUpdate', function(JobInfo)
--     PlayerData.job = JobInfo
-- end)

-- RegisterNetEvent('MP:Client:OnGangUpdate', function(GangInfo)
...
RegisterNUICallback('TrackerError', function(_, cb)
    MP.Functions.Notify("You can't remove your ankle bracelet ..", "error")
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    MP.Functions.Notify("You have deleted your"..data.outfitName.." outfit!")
    cb('ok')
end)
...
AddEventHandler('qb-clothes:client:CreateFirstCharacter', function()
    MP.Functions.GetPlayerData(function(pData)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if data["accessory"] ~= nil then
        if MP.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        MP.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
...
AddEventHandler("qb-clothing:client:adjustfacewear",function(type)
    if MP.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
--  Set Refresh on launch // when player loads in.

function reloadSkin(health)
    local model

    local gender = MP.Functions.GetPlayerData().charinfo.gender
